...
def createStimulus(est, fs):
    ...
    soundFunctions = {'PT':         createPureTone,
                      'IRN':        createIRN,
                      'HCT':        createHCT,
                      'altHCT':     createALTHCT,
                      'chord':      createChord,
                      'noise':      createNoise,
                      'noiseIRN':   createNoiseIRN,
                      'IRNchord':   createIRNchord,
                      'IRNchordSS': createIRNchordSingleSample,
                      'HCTchord':   createHCTchord,
                      'tricky':     createTrickyInterval,
                      'CT':         createClickTrain,
                      'CTchord':    createCTchord,
                      'altCT':      createAltClicks,
                      'IRNseq':     createIRNseq,
                      'FMsweep':    createFMsweep,
                      'vibrato':    createVibratoTone
                      }


    if est['filename'] != -1:
        sound = readStimulus(est,fs)
    elif est['type'] in soundFunctions.keys():
        sound = soundFunctions[est['type']](est, fs)
    else:
...
def createFMsweep(est, fs):
    ...
    tau1 = est['noiseOff']
    tau2 = est.get('transOff', tau1)
    t0   = est['duration'] - tau1 - tau2

    f0 = est['freq'] - 0.5 * est['shift']
    f1 = est['freq'] + 0.5 * est['shift']

    f = np.concatenate((f0 * np.ones(int(tau1 * fs)), \
                        np.tile(np.linspace(f0, f1, int(t0*fs)), est['nOfIts']), \
                        f1 * np.ones(int(tau2 * fs))))
...
def createVibratoTone(est, fs):

    duration = (1. * est['nOfIts']) / est['fVibrato']
    time = np.linspace(0, duration, duration * fs)

    phi = est['noiseOff'] + 2 * np.pi * est['nOfIts']
    vib = np.sin((2 * np.pi * est['fVibrato'] * time + phi))
    f   = 1. + (2.**(1./12) - 1) * vib
