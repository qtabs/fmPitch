function matchDeltasSS(training, resume)

    if nargin < 2, resume = 0; end
    if nargin < 1, training = 0; end
...
function trainSucc = playTraining(pars, mess)
    ...
    blck(:, 2) = blck(:, 2) .* (1 - 0.2 * abs(randn(size(blck(:, 2)))));
...
function playStim(f0, f1, pars)
    ...
    if strcmp(pars.stimSpace, 'inverse')
        fsweep = 1 ./ linspace(1 / f0, 1 / f1, N(2));
    else
        fsweep = linspace(f0, f1, N(2)); 
    end

    f = [];
    for i = 1:pars.reps
        f = [f, f0 * ones([1, N(1)]), fsweep, f1 * ones([1, N(3)])];
    end

    phase = 2 * pi * cumsum(f) / pars.fs;
    stim = (1 / length(f0)) * sin(phase);
    stim = ramps(stim, 0.005, pars.fs);

    sound(stim, pars.fs);
    pause(sum(pars.durations) * pars.reps + 0.01);
...
function saveResults(r, subject, pars)
    ...
    for i = 1:length(avgFreqs)
        A(:, i) = std(results.F{i}(:, 3:5));
    end

    constistency = 1 - (2 * (sqrt(mean(A(:).^2))) / 175);
    fprintf('[Constistency = %.1f]\n\n', constistency);

    if exist('deltaPitchSSDB.mat', 'file') == 0
        matDB.subjects = {};
        save('deltaPitchSSDB', '-struct', 'matDB');
    end

    matDB    = load('deltaPitchSSDB.mat');
...
function [deltas, avgFreqs, testCombs] = getSpaces(pars)
    ...
    if strcmp(pars.fSpace, 'inverse')
        ...
        deltas = linspace(-1000/3, 1000/3, 6);
    end
